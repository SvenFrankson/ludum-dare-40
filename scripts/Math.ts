class LDMath {
    ...
    public static AngleFromToAround(
        ...
        let pFrom: BABYLON.Vector3 = LDMath.ProjectPerpendicularAt(from, around).normalize();
        if (LDMath.IsNanOrZero(pFrom.lengthSquared())) {
            return NaN;
        }
        let pTo: BABYLON.Vector3 = LDMath.ProjectPerpendicularAt(to, around).normalize();
